...
async function getData(filters = {}) {
    ...
    if (response.status == 200) {
        ...
        for (const item of response.data) {
            ...
            div.addEventListener("click", function() { putCurrent(item._id) });
            td.append(div);
            // Delete
            div = document.createElement("div");
        ...
        if (currentCollection == "unicorns" || currentCollection == "auto") {
...
btnDelete.addEventListener("click", async function() {
    ...
    if (confirm("Vuoi veramente cancellare i record " + JSON.stringify(filters) + "?")) {
        ...
        if (response.status == 200) {
            console.log(response.data);
            alert("Eliminazione eseguita con successo " + response.data.deletedCount);
            getData();
        }
        else
            alert(response.status + ": " + response.err);
...
async function patchCurrent(id) {
    const resource = `/${currentCollection}/${id}`;
    ...
    if (response.status == 200) {
        ...
        // Da usare solo su JSON, altrimenti lascio il buco
        delete (current._id);
