...
async function inviaRichiesta(method, url = "", params = {}) {
	...
	if (method == "GET" || method == "DELETE") {
		...
		if (url.includes("?"))
			url += "&" // Sono già presenti dei parametri accodati alla url
		else
			url += "?" // Non sono presenti altri parametri accodati alla url. Aggiungo ?
		url += queryParams.toString()
	}
	else {
		// FormData invia file binari al server (ad esempio immagini)
